...
CHZZK은 공식 Open API를 최종 운영 경로로 사용한다. SOOP은 Goal 1에서 채택한 공개 웹 JSON을 사용하며 변경과 차단 위험을 계약 테스트와 오류 상태로 드러낸다.
...
- 확정 운영 값은 플랫폼별 10분 폴링, CPU 1, 메모리 256MB, PID 128이며 실제 측정 결과는 Goal 7과 `DEPLOYMENT.md`를 따른다.
...
1. C#/.NET 10 전환 계약 사용자 승인.
2. Goal 1 CHZZK 공식 API 실제 응답과 태그 fixture 확보.
3. Goal 1 플랫폼별 초기 호출 정책 결정.

구현 또는 측정 후 확정한 항목.

- CHZZK과 SOOP은 각각 10분 폴링하며 요청 timeout은 5초, 전체 수집 timeout은 60초와 30초다.
- 컨테이너 제한은 CPU 1, 메모리 256MB, PID 128로 시작한다.
- 타일 태그는 최대 3개, 인기 태그 필터는 상위 8개다.
- 현재 규모에서는 태그 인덱스와 단순 부분 문자열 검색으로 충분하다.
- 현재 운영에는 플랫폼별 구조화 성공·실패 로그와 상태 endpoint로 충분하며 별도 metrics endpoint는 도입하지 않는다.
