...
```text
RAIDER_IMAGE=ghcr.io/astrataraxia/raider:1.3.6
...
`.github/workflows/publish-container.yml`은 `v1.3.6` 형태의 Git 태그가 GitHub에 push되면 실행된다.

1. 전체 테스트, 포맷 검사, 경고 오류 빌드를 실행한다.
2. 검증이 통과하면 저장소의 `GITHUB_TOKEN`으로 GHCR에 로그인한다.
3. 예를 들어 `v1.3.6`이면 `ghcr.io/astrataraxia/raider:1.3.6`, `:1.3`, `:latest` 태그를 게시한다.
