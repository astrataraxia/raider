...
Raider `v1.3.6`은 CHZZK과 SOOP의 현재 라이브 방송을 수집하고 검색 가능한 타일 화면, 서버 공용 즐겨찾기, 빠른 수동 새로고침과 전체 문서 reload 없는 부분 갱신을 제공한다. 현재 릴리스 구현과 출시 검증은 완료됐다. 완료 범위와 검증 결과는 [RELEASES.md](../RELEASES.md)에 보관한다.
...
- 플랫폼별 폴링 주기는 10분이다.
- 요청 timeout은 5초, 전체 수집 timeout은 CHZZK 180초와 SOOP 30초다.
