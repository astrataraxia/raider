...
Raider `v2.2.0`은 CHZZK과 SOOP의 현재 라이브 방송을 수집하고 검색 가능한 타일 화면, 서버 공용 즐겨찾기, 빠른 수동 새로고침과 전체 문서 reload 없는 부분 갱신, 그리고 치지직 로그인 이용자를 위한 CHZZK 즐겨찾기 라이브 채팅 리캡(`/recap`)을 제공한다. 한쪽 플랫폼 인증 설정이 없어도 프로세스는 시작되고, 키가 없는 플랫폼만 Configuration 오류로 화면에 표시한다. 브라우저 Console에서 플랫폼별 수집 결과와 소요 시간을 진단할 수 있다. SOOP 수집은 SOOP Developers 공식 API를 사용한다. 현재 릴리스 구현과 출시 검증은 완료됐다. 완료 범위와 검증 결과는 [RELEASES.md](../RELEASES.md)에 보관한다.

현재 진행 중인 Goal은 없다. 후속 작업은 사용자 가치나 운영 문제로 필요성이 확인된 항목만 별도 계획 문서로 만든다.

## 제품 계약.

- CHZZK과 SOOP의 현재 라이브 방송만 수집한다. 채팅은 즐겨찾기 CHZZK 라이브만 추가로 받는다.
- 채팅 본문, 후원 금액, 과거 VOD 채팅은 저장하지 않는다. 남기는 것은 보낸 사람별 날짜 건수와 방송일이다.
- 서버 공용 방송인 즐겨찾기와 CHZZK 채팅 집계만 SQLite에 영구 저장한다. 목록은 요청마다 SQLite에서 읽고, 메모리 캐시와 Redis는 두지 않는다.
- 치지직 OAuth로 로그인한 이용자만 `/recap`에서 자기 집계를 본다. 피라미드·전 이용자 순위·닉네임 공개 검색은 없다.
- 홈 화면 요청은 외부 플랫폼을 호출하지 않고 현재 메모리 스냅샷만 읽는다. `/recap`은 SQLite 집계만 읽는다.
...
- 즐겨찾기 SQLite 파일만 서버 호스트 bind mount에 영구 저장한다. Redis와 즐겨찾기 메모리 캐시는 범위 밖이다.
...
- CHZZK 라이브 목록과 로그인(`users/me`)은 공식 Open API를 사용한다. 즐겨찾기 채널 채팅은 읽기 전용 비공식 소켓이다. 공식 Session은 Socket.IO라 시청자 리캡 수집 경로로 쓰지 않는다.
