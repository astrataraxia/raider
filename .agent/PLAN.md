...
Raider `v1.3.9`은 CHZZK과 SOOP의 현재 라이브 방송을 수집하고 검색 가능한 타일 화면, 서버 공용 즐겨찾기, 빠른 수동 새로고침과 전체 문서 reload 없는 부분 갱신을 제공한다. 브라우저 Console에서 플랫폼별 수집 결과와 소요 시간을 진단할 수 있다. 현재 릴리스 구현과 출시 검증은 완료됐다. 완료 범위와 검증 결과는 [RELEASES.md](../RELEASES.md)에 보관한다.
